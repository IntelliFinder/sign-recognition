...
class SelectorBIC(ModelSelector):
    ...
    def select(self):
        ...
        for num_components in range(self.min_n_components, self.max_n_components):
            ...
            bic = -2*logL + len(self.X)*np.log(len(words))
...
class SelectorDIC(ModelSelector): #goal : rough sketch
    ...
    def select(self):
        warnings.filterwarnings("ignore", category=DeprecationWarning)
        # TODO implement model selection using DIC
        ...
        for num_components in range(self.min_n_components, self.max_n_components):
            ...
            model = self.base_model(num_components).fit(self.X, self.lengths)
            anti_model = self.base_model(num_components).fit(other_X, other_lengths) #fill in values
            log_l = model.score(self.X, self.lengths)
            log_anti_l = model.score(self.X, self.lengths)
            np.log(log_l) - 1/(M-1)sum(np.log(log_anti_l) 
        raise NotImplementedError


class SelectorCV(ModelSelector): #goal: pass testing
    ...
    def select(self):
        ...
        kf = KFold() #default check necesseity to change to other value
        for num_components in range(self.min_n_components, self.max_n_components):
            log_l_fold = []
            for split_index_list, test_index in kf.split(self.sequences):
                #combine sequences from asl_recognizer
                X, lengths=combine_sequences(split_index_list, self.sequences) #go to asl_utils
                x_test, lengths_test = combine_sequences( test_index, self.sequences )
                #build model
                model = self.build_model(num_components).fit(X, lengths)
                #score with test idx
                curr_log_l = model.score(X_test, lengths_test)
